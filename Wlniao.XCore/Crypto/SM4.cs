...
namespace Wlniao.Crypto
{
	...
	public class SM4
	{
		...
		private byte[] SboxTable = new byte[] { (byte) 0xd6, (byte) 0x90, (byte) 0xe9, (byte) 0xfe,
		...
		private uint[] CK = { 0x00070e15,0x1c232a31,0x383f464d,0x545b6269,
		...
		/// <returns></returns>
		public byte[] EncryptECB(byte[] plainBytes, byte[] keyBytes, bool isPadding = true)
		{
		...
		/// <param name="isPadding"></param>
		/// <returns></returns>
		public byte[] DecryptECB(byte[] encryBytes, byte[] keyBytes, bool isPadding = true)
		{
			var skBytes = new long[32];
			sm4_setkey(skBytes, keyBytes);
			for (var i = 0; i < 16; i++)
			...
			var plainBytes = sm4_crypt_ecb(SM4_DECRYPT, isPadding, skBytes, encryBytes);
			return plainBytes;
		}
		/// <summary>
		/// 
		...
		/// <param name="isPadding"></param>
		/// <returns></returns>
		public byte[] EncryptCBC(byte[] plainBytes, byte[] keyBytes, byte[] ivBytes, bool isPadding = true)
		{
			var skBytes = new long[32];
			sm4_setkey(skBytes, keyBytes);
		...
		/// <param name="isPadding"></param>
		/// <returns></returns>
		public byte[] DecryptCBC(byte[] encryBytes, byte[] keyBytes, byte[] ivBytes, bool isPadding = true)
		{
			var skBytes = new long[32];
			sm4_setkey(skBytes, keyBytes);
			for (var i = 0; i < 16; i++)
			...
			var plainBytes = sm4_crypt_cbc(SM4_DECRYPT, isPadding, skBytes, ivBytes, encryBytes);
			return plainBytes;
		}
	}
}