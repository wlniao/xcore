...
namespace Wlniao.IO
{
    /// <summary>
    /// 检测字符编码的类
    /// <seealso cref="System.Byte">字节数组</seealso>
    /// <seealso cref="System.SByte">字节数组</seealso>
    /// <seealso cref="System.String">文件路径</seealso>
    /// </summary>
    public class IdentifyEncoding
    {
        /// <summary>
        /// 从指定的 <see cref="System.String"/> 文件路径中判断编码类型
        /// </summary>
        ...
        /// </summary>
        /// <param name="buffer">要判断的 <see cref="System.Byte"/> 字节数组</param>
        /// <returns>返回编码类型("GB2312", "GBK", "HZ", "Big5", "CNS 11643", "ISO 2022CN", "UTF-8", "Unicode", "ASCII", "OTHER")</returns>
        ...
        /// </summary>
        /// <param name="buffer">要判断的 <see cref="System.SByte"/> 字节数组</param>
        /// <returns>返回编码类型("GB2312", "GBK", "HZ", "Big5", "CNS 11643", "ISO 2022CN", "UTF-8", "Unicode", "ASCII", "OTHER")</returns>
        public static string GetEncodingName(sbyte[] buffer)
        {
            ...
            //分析编码的概率
            var scores = new int[] {
                GB2312Probability(GBFreq, buffer)
                ,
                GBKProbability(GBFreq, GBKFreq, buffer)
                ,
                HZProbability(GBFreq, buffer)
                ,
                BIG5Probability(Big5Freq, buffer)
                ,
                ENCTWProbability(EUC_TWFreq, buffer)
                ,
                ISO2022CNProbability(GBFreq, EUC_TWFreq, buffer)
                ,
                UTF8Probability(buffer)
                ,
                UnicodeProbability(buffer)
                ,
                ASCIIProbability(buffer)
            };

            var maxScore = 0;   //最大分值
            var maxIndex = 0;   //最大分值的序号
            ...
            if (maxScore <= 50)
            {
                return "OTHER"; // Return OTHER if nothing scored above 50
            }
            else
            {
                return new string[] { "GB2312", "GBK", "HZ", "Big5", "CNS 11643", "ISO 2022CN", "UTF-8", "Unicode", "ASCII" }[maxIndex];
            }
        }
        ...
        private static int GB2312Probability(int[][] GBFreq, sbyte[] rawtext)
        {
            int i, rawtextlen = 0;
        ...
        /// <param name="rawtext">要判断的 <see cref="sbyte"/> 字节数组</param>
        /// <returns>返回 0 至 100 之间的可能性</returns>
        private static int GBKProbability(int[][] GBFreq, int[][] GBKFreq, sbyte[] rawtext)
        {
            int i, rawtextlen = 0;
        ...
        /// <param name="rawtext">要判断的 <see cref="sbyte"/> 字节数组</param>
        /// <returns>返回 0 至 100 之间的可能性</returns>
        private static int HZProbability(int[][] GBFreq, sbyte[] rawtext)
        {
            int i, rawtextlen;
        ...
        /// <param name="rawtext">要判断的 <see cref="sbyte"/> 字节数组</param>
        /// <returns>返回 0 至 100 之间的可能性</returns>
        private static int BIG5Probability(int[][] Big5Freq, sbyte[] rawtext)
        {
            int i, rawtextlen = 0;
        ...
        /// <param name="rawtext">要判断的 <see cref="sbyte"/> 字节数组</param>
        /// <returns>返回 0 至 100 之间的可能性</returns>
        private static int ENCTWProbability(int[][] EUC_TWFreq, sbyte[] rawtext)
        {
            int i, rawtextlen = 0;
        ...
        /// <param name="rawtext">要判断的 <see cref="sbyte"/> 字节数组</param>
        /// <returns>返回 0 至 100 之间的可能性</returns>
        private static int ISO2022CNProbability(int[][] GBFreq, int[][] EUC_TWFreq, sbyte[] rawtext)
        {
            int i, rawtextlen = 0;
