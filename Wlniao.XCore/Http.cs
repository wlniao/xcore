/*==============================================================================
    文件名称：Http.cs
    适用环境：CoreCLR 5.0,.NET Framework 2.0/4.0
    功能描述：基于Socket的HTTP服务端
================================================================================
    ...
    public class Http
    {
        ...
        public class Context
        {
            ...
            public String ContentType
            {
                ...
                set
                {
                    contentType = string.IsNullOrEmpty(value) ? "application/json" : value;
                }
            }
        }
        ...
        public void Start()
        {
            ...
            listener.Listen();
            Console.WriteLine("Now listening on: http://127.0.0.1:" + ListenPort, ConsoleColor.DarkGreen);
            while (true)
            {
                ...
                Task.Run(() =>
                {
                    try
                    {
                        ...
                        if (receiveNumber > 0)
                        {
                            ...
                            var msg = Encoding.ASCII.GetString(request, 0, receiveNumber);
                            ...
                            if (ctx.Request != null)
                            {
                                ...
                                else
                                {
                                    ...
                                    res = System.Text.UTF8Encoding.UTF8.GetString(System.Text.Json.JsonSerializer.SerializeToUtf8Bytes(value: ctx.Response));
                                }
                                sb.Append("Content-Type: " + ctx.ContentType + "\r\n");
                                sb.Append("Content-Length: " + res.Length + "\r\n");
                                sb.Append("\r\n" + res);
                            }
                            msg = sb.ToString();
